...
### エラーダイアログ
![エラーダイアログ](images/add-page-recipe-error-dialog.png)  
登録処理に失敗した場合、エラーメッセージが記載されたダイアログが表示されます。  
エラーの具体例として、ネットワーク障害やサーバー側の処理エラーなどが発生した際に表示されることがあります。  
ユーザーは「CLOSE」ボタンを押すことでダイアログを閉じ、入力内容を修正したうえで再度SUBMITボタンを押すことができます。
